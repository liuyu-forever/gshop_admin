  ...
  methods: {
    ...
    */
    async showSkuInfo(id) {
      ...
      const result = await this.$API.sku.getSkuInfoById(id)
    ...
    */
    async getSkuList(page = 1) {
      this.page = page
      this.loading = true
      const result = await this.$API.sku.getSkuInfoList(this.page, this.limit)
    ...
    */
    async deleteSku(skuId) {
      const result = await this.$API.sku.deleteSkuById(skuId)
      if (result.code === 200) {
        this.$message({
          message: '删除SKU成功',
          type: 'success',
